...
const Page = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await parrainApi.getParrainById(params.id);
        const parrain = response.parrain;
        console.log("Parrain data:", parrain); // Debug log
        ...
        setForm({
          ...
          departement: parrain.departement || "",
        });
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    // Validation
    if (!form.nom || !form.prenom || !form.departement) {
    ...
    try {
      ...
      await parrainApi.updateParrain(params.id, updateData);
      setSuccess("Parrain mis à jour avec succès!");
      
      // Redirect to parrain list page after a short delay
      setTimeout(() => {
        router.push("/parrain");
      }, 1500);
    } catch (err) {
      console.error("Erreur lors de la mise à jour du parrain:", err);
      setError("Erreur lors de la mise à jour du parrain");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async () => {
    if (!confirm("Êtes-vous sûr de vouloir supprimer ce parrain ? Cette action est irréversible.")) {
      return;
    }

    setLoading(true);
    try {
      await parrainApi.deleteParrain(params.id);
      setSuccess("Parrain supprimé avec succès!");
      setTimeout(() => {
        router.push("/parrain");
      }, 1500);
    } catch (err) {
      console.error("Erreur lors de la suppression:", err);
      setError("Erreur lors de la suppression du parrain");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="w-full p-6 bg-gray-100 min-h-screen">
      ...
      >
        <legend className="text-blue-600 text-2xl font-bold mb-6">Modifier le Parrain #{params.id}</legend>
        ...
        <div className="flex flex-col gap-2">
          ...
          <input 
            ...
            onChange={handleChange} 
            className="border border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            placeholder="Département du parrain"
            required
          />
        </div>
