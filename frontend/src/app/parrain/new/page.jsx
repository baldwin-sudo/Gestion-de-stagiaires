...
const Page = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch("http://localhost:8080/parrain/", {
        method: "POST",
        body: formData,
      });
  ...
  return (
    <div className="w-full p-6 bg-gray-100 min-h-screen">
      ...
      >
        <legend className="text-blue-600 text-xl mb-4">Nouveau Parrain</legend>
        ...
        <div className="flex flex-col gap-2">
          ...
          <input name="departement" value={form.departement} onChange={handleChange} required className="border rounded px-3 py-2" />
        </div>
        <button
          type="submit"
        ...
        {status === "success" && <p className="text-green-600">Parrain créé !</p>}
