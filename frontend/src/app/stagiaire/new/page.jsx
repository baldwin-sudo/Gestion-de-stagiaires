...
function Page () {
  ...
  const handleSubmit = async (e) => {
    ...
    setStatus("loading");
    try {
      const res = await fetch("http://localhost:8080/stagiaire/", {
        method: "POST",
      ...
      setStatus("success");
      setForm({ nom: "", prenom: "", ecole: "", specialite: "" });
    } catch (err) {
      setStatus("error");
    }
  };
  ...
  return (
    <div className="w-full p-6 bg-gray-100 min-h-screen">
      ...
      >
        <legend className="text-blue-600 text-xl mb-4">Nouveau Stagiaire</legend>
        ...
        <div className="flex flex-col gap-2">
          <label>École</label>
          <input name="ecole" value={form.ecole} onChange={handleChange} className="border rounded px-3 py-2" />
        </div>
        <div className="flex flex-col gap-2">
          <label>Spécialité</label>
          <input name="specialite" value={form.specialite} onChange={handleChange} className="border rounded px-3 py-2" />
        </div>
        <button
          type="submit"
        ...
        {status === "success" && <p className="text-green-600">Stagiaire créé !</p>}
